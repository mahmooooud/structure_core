...
import 'package:structure_core/core/model/error_model/base_error_list_model.dart';
import 'package:structure_core/core/model/error_model/base_error_model.dart';
import 'package:structure_core/core/ui/show_error.dart';
      ...
      switch (method) {
        ...
        case HttpMethod.POST:
          response = await _client.post(
            ...
            data: body,
            queryParameters: queryParameters,
            options: options ?? GetIt.I<DioCacheOptions>()(
    ...
    if (error.type == DioErrorType.other ||
      ...
      if (error.type == DioErrorType.response) {
        if (error.response!.data != null) {
          try {
            if(error.response!.statusCode == 400){
              errorListModel = List<BaseErrorListModel>.from(error.response!.data.map((model){
                return BaseErrorListModel.fromJson(json.decode(json.encode(model)) );
              })).toList();
            }else{
        ...
        switch (error.response!.statusCode) {
          case 400:
            if (errorListModel != null || (errorListModel?.isNotEmpty ?? false)) {
              if (errorListModel?.first.errorMessage != null) {
                String? message = AppConfig().currentLanguageCode == 'ar'
                    ? LangReader.getArValue(errorListModel?.first.errorMessage ?? "")
                    : LangReader.getEnValue(errorListModel?.first.errorMessage ?? "");
                return BadRequestError(
                    message: message ?? errorListModel?.first.errorMessage ?? "");
              }
            }
          ...
          case 404:
            if (errorModel != null) {
              if (errorModel.exCode != null) {
                String? message = AppConfig().currentLanguageCode == 'ar'
                    ? LangReader.getArValue(
                        errorModel.message!.first.errorMessage!)
                    : LangReader.getEnValue(
                        errorModel.message!.first.errorMessage!);
                return BadRequestError(
