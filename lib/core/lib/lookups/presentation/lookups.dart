  ...
  void initState() {
    if (widget.items == null) {
      if (widget.httpMethod == HttpMethod.GET) {
        ...
        controller.getLookups(widget.url, HttpMethod.POST,);
      }
    } else {
      if (kDebugMode) {
              ...
              child: DropdownSearch<LookupsDataModel>(
                ...
                popupProps: PopupProps.bottomSheet(
                  searchFieldProps: TextFieldProps(
                    decoration: InputDecoration(
                      ...
                      enabledBorder: OutlineInputBorder(
                        ...
                        borderSide:BorderSide(color: context.colors.grey2, width: 1),
                      ),
                  ...
                  itemBuilder: (con, model, b) {
                    return Container(
                      ...
                      child: CustomText(
                       model.fullName != null ? model.fullName ?? "" : (AppConfig().currentLanguageCode == 'ar')
                            ? model.nameAr!
                            : model.nameEn!,
                ...
                dropdownBuilder: (con, model) {
                  return CustomText(
                      (model == null)
                          ?  "${Translate.s.select} ${widget.label}"
                          : model.fullName?.isNotEmpty ?? false ? model.fullName ?? "": (AppConfig().currentLanguageCode == 'ar')
                              ? model.nameAr ?? ""
                              : model.nameEn ?? "",
                      textAlign: TextAlign.start,
                      textStyle: context.textTheme.bodyText1!.copyWith(
                ...
                dropdownDecoratorProps: DropDownDecoratorProps(
                  dropdownSearchDecoration: widget.decoration ?? InputDecoration(
                    contentPadding: EdgeInsetsDirectional.only(start: 10),
                    enabledBorder: OutlineInputBorder(
                      borderRadius: BorderRadius.all(Radius.circular(10.r)),
                      borderSide:
                    ...
                    focusedErrorBorder:  OutlineInputBorder(
                      borderRadius: BorderRadius.all(Radius.circular(8.r)),
                      borderSide: BorderSide(color: context.colors.red, width: 1),
                    ),
                    disabledBorder: OutlineInputBorder(
                ...
                (item.nameEn?.toLowerCase().contains(filter) ?? false ||
                        (item.nameAr?.toLowerCase().contains(filter)?? false))  || (item.fullName?.toLowerCase().contains(filter)?? false),
              ),
            ),
          ),
