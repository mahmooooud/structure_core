...
/// Wrap the widgets to show a scroll to top prompt over when a certain scroll
/// offset is reached.
class ScrollWrapper extends StatefulWidget {
  const ScrollWrapper({
    ...
    this.promptDuration = const Duration(milliseconds: 400),
    this.promptAnimationCurve = Curves.elasticInOut,
  ...
  /// [ScrollController] of the scrollable widgets to scroll to the top of when
  /// the prompt is pressed.
  final ScrollController? scrollController;

  /// The widgets to show the prompt over when scroll is at an offset.
  ...
  ///
  /// For structure_core, if the reading direction is left-to-right and
  ...
  /// **Replace the prompt button with your own custom widgets. Returns the**
  ...
  /// Where on the widgets to align the prompt.
  final AlignmentGeometry? promptAlignment;
...
class ScrollWrapperState extends State<ScrollWrapper> {
  ...
  late AlignmentGeometry _promptAlignment;

  @override
  void initState() {
    super.initState();
    _promptTheme = widget.promptTheme ?? PromptButtonTheme(
      icon: Icon(Icons.arrow_upward, color: context.colors.primary,size: 15.sp,),
  ...
  AlignmentGeometry _setPromptAlignment() {
