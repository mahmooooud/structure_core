...
import 'package:structure_core/core/constants/enums/mime_type_enum.dart';
...
class FileService{
  ...
  bool isSizeLessThanRequiredSize(int size,int requiredSize,[int fractionDigits = 2]) {
    if (size <= 0) return false;
    //  num newSize = double.parse((size / pow(1024, 2)).toStringAsFixed(fractionDigits));
  ...
  MediaType? getMimeTypeFromPath(String filePath) {
    ...
    MimeTypeEnum? acceptType = MimeTypeEnum.values.firstWhereOrNull((element) => element.mediaType == type);
    if(acceptType != null){
