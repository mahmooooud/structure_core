...
import 'package:structure_core/core/common/extension/date_extension.dart';
...
List<String> getMonthsInYear() {
  ...
  while (date.isBefore(allMonths)) {
    dates.add(
        DateFormat.MMMM((AppConfig().appLocal == null) ? 'en' : AppConfig().appLocal!.languageCode)
            .format(date));
...
String dateTimeRangeFormat(dynamic val) {
  ...
  if (val == null) {
    ...
    value = (('${val[0].toString().parsDateTime!.formatDateAndTime} - ${val[1].toString().parsDateTime!.formatDateAndTime}'));
  }
  return value;
}
...
String dateRangeFormat(dynamic val) {
  ...
  if (val == null) {
    ...
    if (val.length > 1) {
      value = (('${val[0].toString().parsDateTime!.formatDate} - ${val[1].toString().parsDateTime!.formatDate}'));
    }
  }
  return value;
}
...
String monthYearRangeFormat(dynamic val) {
  ...
  if (val == null) {
    ...
    if (val.length > 1) {
      value = (('${val[0].toString().parsDateTime!.formatMonth} - ${val[1].toString().parsDateTime!.formatMonth}'));
    }
  }
  return value;
}
